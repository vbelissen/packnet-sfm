...
class ImageBasedValeoDataset(Dataset):
    ...
    def _read_rgb_context_files(self, session, filename):
        context_paths = self._get_context_file_paths(filename)
        return [load_convert_image(os.path.join(self.root_dir, session, filename))
    ...
    def __getitem__(self, idx):
        ...
        sample = {
            ...
            #
            'rgb': image,
            'intrinsics': perspective_calibration()#dummy_calibration(image)
        }
