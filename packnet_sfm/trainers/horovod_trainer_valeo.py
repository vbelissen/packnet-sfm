...
class HorovodTrainer(BaseTrainer):
    ...
    def test(self, module, filename):
        # Send module to GPU
        module = module.to('cuda', dtype=self.dtype)
        # Get test dataloaders
        test_dataloaders = module.test_dataloader()
        # Run evaluation
        self.evaluate(test_dataloaders, module, filename)

    @torch.no_grad()
    def evaluate(self, dataloaders, module, filename):
        # Set module to eval
        module.eval()
        ...
        for n, dataloader in enumerate(dataloaders):
            ...
            all_outputs.append(outputs)
        # Return all outputs for epoch end
        return module.test_epoch_end(all_outputs, filename)
