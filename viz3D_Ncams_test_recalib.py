...
def infer_plot_and_save_3D_pcl(input_files, output_folder, model_wrappers, image_shape, half, save, stop):
    ...
    for K in values:
        ...
        for i_cam in range(N_cams):
            ...
            ind_clean.append(list(set.intersection(set(list(np.where(mask)[0])), set(ind))))
        ...
        if color_cam:
            for i_cam in range(4):
                if i_cam == 0:
                    pcl_full[i_cam].paint_uniform_color([1.0, 0.0, 0.0])#.colors = o3d.utility.Vector3dVector(images_numpy[i_cam])
                elif i_cam == 1:
        ...
        for i_cam in range(4):
            ...
            far_distance = 5
            mask_far = np.sqrt(np.square(points_tmp[:, 0] - cams_middle[0]) + np.square(points_tmp[:, 1] - cams_middle[1])) < far_distance
            ind_clean_left.append(list(set.intersection(set(list(np.where(mask_far  * maskX_left  * maskY_left)[0])),  set(ind_clean[i_cam]))))
            ind_clean_right.append(list(set.intersection(set(list(np.where(mask_far * maskX_right * maskY_right)[0])), set(ind_clean[i_cam]))))
