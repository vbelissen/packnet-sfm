...
front_next_img_torch_small = interpolate_image(front_next_img_torch, new_shape, mode='bilinear', align_corners=True)
left_img_torch_small       = interpolate_image(left_img_torch,       new_shape, mode='bilinear', align_corners=True)
right_img_torch_small      = interpolate_image(right_img_torch,      new_shape, mode='bilinear', align_corners=True)

simulated_depth_small      = interpolate_image(simulated_depth,      new_shape, mode='bilinear', align_corners=True)
...
cam_front = cam_front.scaled(scale_factor).to(torch.device('cuda'))
cam_left  = cam_left.scaled(scale_factor).to(torch.device('cuda'))

world_points = cam_front.reconstruct(simulated_depth_small,frame='w')

ref_coords_left = cam_left.project(world_points, frame='w')

warped_front_left = funct.grid_sample(left_img_torch_small, ref_coords_left, mode='bilinear', padding_mode='zeros', align_corners=True)
...
cam_front = cam_front.scaled(scale_factor).to(torch.device('cuda'))
cam_right = cam_right.scaled(scale_factor).to(torch.device('cuda'))
