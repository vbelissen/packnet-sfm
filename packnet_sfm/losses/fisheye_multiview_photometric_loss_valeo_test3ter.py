    ...
    def forward(self, image, context, inv_depths,
        ...
        for j, (ref_image, pose) in enumerate(zip(context, poses)):
            ...
            if self.automask_loss:
                ...
                if self.mask_ego:
                    unwarped_image_loss = 2 * self.calc_photometric_loss([a * b for a, b in zip(ref_images, ref_ego_mask_tensors[j])],
                                                                     [a * b for a, b in zip(images,     ego_mask_tensors)])
                else:
                    unwarped_image_loss = 2 * self.calc_photometric_loss(ref_images, images)
                for i in range(self.n):
