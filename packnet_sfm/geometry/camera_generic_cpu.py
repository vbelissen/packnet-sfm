...
class GenericCamera_cpu(nn.Module):
    """
...
########################################################################################################################
    ...
    def project(self, X, progress, downsample=True, frame='c'):
        ...
        def _make_grid(xi, xf, yi, yf, r=1., final=False):
            ...
            return torch.stack([x, y], 2).long().to(device)#.cuda()
        ...
        patch_coordinates = _get_patch_coords(
            H, W, patch_side, patch_side).to(device)#.cuda()
        ray_surf_patch = _get_value_coords(
            ray_surface, patch_coordinates).to(device)#.cuda()
