...
class KITTIBasedValeoDatasetFisheye(Dataset):
    """
...
########################################################################################################################
    ...
    def _get_intrinsics(self, image_file, calib_data):
        ...
        for cam in self.cameras:
            ...
            if cam in image_file or CAM_NAMES_CONVERT[cam] in image_file:
                ...
                scale_factors = np.array([1., 1./float(intr['pixel_aspect_ratio'])])
    ...
    @staticmethod
    def _get_path_to_theta_lut(image_file):
        """Get the current folder from image_file."""
        return os.path.join(os.path.dirname(image_file), 'theta_tensor_1280_800.npy')
