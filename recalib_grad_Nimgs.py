...
def infer_optimal_calib(input_files, model_wrappers, image_shape, half):
    ...
    for epoch in range(n_epochs):
        ...
        for i_file in range(N_files):
            ...
            #print(extra_rot_deg_list)
            with torch.no_grad():
                extra_rot_deg_list.append(extra_rot_deg.grad.clone())
                extra_rot_deg.grad.zero_()
                loss_sum += loss.item()
                print(loss)

        with torch.no_grad():
            extra_rot_deg.sub_(sum(extra_rot_deg_list)/N_files*1.0)
            print('End of epoch')
            print(extra_rot_deg)

        loss_tab[epoch] = loss_sum
