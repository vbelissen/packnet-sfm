...
@torch.no_grad()
def infer_plot_and_save_3D_pcl(input_files, output_folder, model_wrappers, image_shape, half, save):
    """
    Process a single input file to produce and save visualization
    ...
    for i_cam in range(N_cams):
        os.makedirs(os.path.join(output_folder, camera_names[i_cam]), exist_ok=True)
    ...
    for i_file in range(0, N_files, 25):
        ...
        for i_cam in range(N_cams):
            ...
            cl, ind = pcl_full[i_cam].remove_statistical_outlier(nb_neighbors=5, std_ratio=1.2)
            points = np.asarray(pcl_full[i_cam].select_by_index(ind).points)
            # remove points that are above but close (sky...)
            mask = 1 - (points[:, 2] > 2.5) * (abs(points[:, 0]) < 10) * (abs(points[:, 1]) < 3)
            pcl = pcl_full[i_cam].select_by_index(ind).select_by_index(np.where(mask)[0])
            ...
            if has_gt_depth[i_cam]:
                ...
                pcl_gt[i_cam].paint_uniform_color([1.0, 0.0, 0])
            else:
                pcl_gt.append(0)
        ...
        ctr.set_up(up_vector)
        ctr.set_zoom(zoom_float)
        param = o3d.io.read_pinhole_camera_parameters('/home/vbelissen/Downloads/test.json')
        ...
        plt.imsave(os.path.join(output_folder, 'image_' + str(i_file) + '.png'), np.asarray(image), dpi=1)
        ...
        #o3d.visualization.draw_geometries(pcl_only_inliers + [e for i, e in enumerate(pcl_gt) if e != 0])
        #o3d.visualization.draw_geometries(pcl_only_inliers + pcl_only_outliers + [e for i, e in enumerate(pcl_gt) if e != 0])
        #o3d.visualization.draw_geometries([i.crop(bbox) for i in pcl_only_inliers] + [e for i, e in enumerate(pcl_gt) if e != 0])
        ...
        for i_cam in range(N_cams):
            ...
            viz_pred_inv_depths.append(viz_inv_depth(pred_inv_depths[i_cam][0]) * 255)
            ...
            # Save visualization
            output_file = os.path.join(output_folder, camera_names[i_cam], os.path.basename(input_files[i_cam][i_file]))
            print(output_file)
            print('Saving {} to {}'.format(
                pcolor(input_files[i_cam][i_file], 'cyan', attrs=['bold']),
                pcolor(output_file, 'magenta', attrs=['bold'])))
            imwrite(output_file, concat[:, :, ::-1])
...
def main(args, N):
    ...
    n_files = len(files[0])
    # Process each file
    infer_plot_and_save_3D_pcl(files, args.output, model_wrappers, image_shape, args.half, args.save)
