    ...
    for i_file in range(N_files):
        ...
        def photo_loss_2imgs(i_cam1, i_cam2, rot_vect_list, save_pictures, rot_str):
            ...
            for i, i_cam in enumerate([i_cam1, i_cam2]):
                ...
                calib_data = {}
                calib_data[camera_str] = read_raw_calib_files_camera_valeo(base_folder_str, split_type_str, seq_name_str, camera_str)
                pose_matrix = torch.from_numpy(get_extrinsics_pose_matrix_extra_rot(input_files[i_cam][i_file],
                                                                                    calib_data,
                                                                                    rot_vect_list[i][0],
                                                                                    rot_vect_list[i][1],
                                                                                    rot_vect_list[i][2])).unsqueeze(0)
            ...
            if save_pictures:
                ...
                cv2.imwrite('/home/vbelissen/Downloads/cam_' + str(i_cam2) + '_recons_from_' + str(i_cam2) + '_rot' + rot_str + '.png',
        ...
        def photo_loss_4imgs(extra_rot_deg, save_pictures=True):
            ...
            for i_pair in range(4):
                ...
                for i_rot in range(12):
                    rot_str = rot_str + '_' + str(int(10 * extra_rot_deg[i_rot])/10)

                loss += photo_loss_2imgs(idx_cam1,
                                         idx_cam2,
                                         [extra_rot_deg[3 * idx_cam1:3 * (idx_cam1 + 1)], extra_rot_deg[3 * idx_cam2:3 * (idx_cam2 + 1)]],
                                         save_pictures,
                                         rot_str)
        ...
        result = minimize(photo_loss_4imgs, x0=np.zeros(12), method='Powell', options={'disp': True})
        print(result)
