...
from packnet_sfm.utils.image_valeo import image_grid, centered_2d_grid, centered_2d_grid_woodscape, meshgrid


########################################################################################################################

class CameraFisheyeWoodscape(nn.Module):
    """
    ...
    def to(self, *args, **kwargs):
        ...
        #self.scale_factor_y = self.scale_factor_y.to(*args, **kwargs)
...
########################################################################################################################
    ...
    def scaled(self, x_scale, y_scale=None):
        ...
        return CameraFisheyeWoodscape(path_to_theta_lut, poly_coeffs, principal_point, self.scale_factor_y, Tcw=self.Tcw)
...
########################################################################################################################
    ...
    def project(self, X, frame='w'):
        ...
        v = rho * torch.sin(phi) * self.scale_factor_y.unsqueeze(1) + self.principal_point[:, 1].unsqueeze(1) # [B, HW]
