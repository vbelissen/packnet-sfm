    ...
    for epoch in range(n_epochs):
        ...
        for i_file in range(N_files):
            ...
            def photo_loss_2imgs(i_cam1, i_cam2, rot_vect_list, save_pictures, rot_str):
                ...
                if save_pictures:
                    if epoch == 0:
                        cv2.imwrite('/home/vbelissen/Downloads/cam_' + str(i_cam1) + '_' + str(i_file) + '_orig.png',
                                    torch.transpose((images[i_cam1][0, :, :, :]).unsqueeze(0).unsqueeze(4), 1, 4).squeeze().detach().cpu().numpy() * 255)
                        cv2.imwrite('/home/vbelissen/Downloads/cam_' + str(i_cam2) + '_' + str(i_file) + '_orig.png',
                                    torch.transpose((images[i_cam2][0, :, :, :]).unsqueeze(0).unsqueeze(4), 1, 4).squeeze().detach().cpu().numpy() * 255)
                    cv2.imwrite('/home/vbelissen/Downloads/cam_' + str(i_cam1) + '_' + str(i_file) + '_recons_from_' + str(i_cam2) + '_rot' + rot_str + '.png',
                                torch.transpose(((reconstructedImg2to1*not_masked[i_cam1])[0, :, :, :]).unsqueeze(0).unsqueeze(4), 1, 4).squeeze().detach().cpu().numpy() * 255)
                    cv2.imwrite('/home/vbelissen/Downloads/cam_' + str(i_cam2) + '_' + str(i_file) + '_recons_from_' + str(i_cam1) + '_rot' + rot_str + '.png',
