        ...
        for j, (ref_image, pose) in enumerate(zip(context, poses)):
            ...
            if self.mask_ego:
                ref_warped, ref_ego_mask_tensors_warped = self.warp_ref_image_tensor(inv_depths, ref_image,
                                                                                     ...
                                                                                     path_to_theta_lut,
                                                                                     path_to_ego_mask, poly_coeffs,
                                                                                     principal_point, scale_factors,
                                                                                     ref_path_to_theta_lut[j],
                                                                                     ref_path_to_ego_mask[j],
                                                                                     ref_poly_coeffs[j],
                                                                                     ref_principal_point[j],
                                                                                     ref_scale_factors[j],
                                                                                     same_timestep_as_origin[j],
                                                                                     pose_matrix_context[j],
                                                                                     pose)
                photometric_loss = self.calc_photometric_loss([a * b * c for a, b, c in zip(ref_warped, ref_ego_mask_tensors_warped)],
                                                              [a * b * c for a, b, c in zip(images,     ego_mask_tensors)])
            else:
