...
rz1_front_rad = 267.6573 * np.pi/180 + np.pi
rz2_front_rad = -1.9011  * np.pi/180+ np.pi

rx_left_rad   = 52.6295  * np.pi/180
rx_left_mod   = rx_left_rad + np.pi
rz1_left_rad  = 355.598  * np.pi/180 + np.pi
rz2_left_rad  = -1.36427 * np.pi/180+ np.pi

rx_right_rad   = 52.2021  * np.pi/180
rx_right_mod   = rx_right_rad + np.pi
rz1_right_rad  = 176.26216  * np.pi/180 + np.pi
rz2_right_rad  = 0.691546 * np.pi/180+ np.pi
...
R_front   = np.matmul(Rz1_front, np.matmul(Rx_front, Rz2_front))
R_left    = np.matmul(Rz1_left,  np.matmul(Rx_left,  Rz2_left))
R_right   = np.matmul(Rz1_right,  np.matmul(Rx_right, Rz2_right))
...
relative_pose_left = invert_pose_numpy(invert_pose_numpy(pose_matrix_left) @ pose_matrix_front)

relative_pose_left_torch = torch.zeros(1,4,4)
relative_pose_left_torch[0,:,:] = torch.from_numpy(relative_pose_left)

relative_pose_right = invert_pose_numpy(invert_pose_numpy(pose_matrix_right) @ pose_matrix_front)

relative_pose_right_torch = torch.zeros(1,4,4)
relative_pose_right_torch[0,:,:] = torch.from_numpy(relative_pose_right)
...
cam_right = CameraFisheye(path_to_theta_lut=['/home/data/vbelissen/valeo_multiview/calibrations_theta_lut/fisheye/test/20170320_163113/20170320_163113_cam_0_1280_800.npy'],
                         ...
                         principal_point=torch.from_numpy(np.array([[0.046296,-7.33178]])).float(),
                         scale_factors=torch.from_numpy(np.array([[1.0,1.0]])).float(),
                         Tcw=Pose(relative_pose_right_torch))
